...
async function getDATA() {
    ...
    const data = await fetch("http://localhost/wordpressOSKAR/wp-json/wc/v3/products", ({
        method: "GET",
        headers: {
    ...
    for (let i in json) {
        ...
        button_change.addEventListener("onclick", () => {
...
async function changeQuantity(id, stock_quantity, quantity) {
    ...
    const url = new URL(`http://localhost/wordpressOSKAR/wp-json/wc/v3/products/${id}`)
